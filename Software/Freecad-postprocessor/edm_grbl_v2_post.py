...
TOOLTIP = """
Generate g-code from a Path that is compatible with the gedm
"""
...
OUTPUT_HEADER = False  # default output header in output gCode file
...
def parse(pathobj):
    ...
    global Z_IS_UP
    global Z_ONLY_COMMAND
    global LAST_OUTSTRING
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
        ...
        for c in pathobj.Path.Commands:
            ...
            for param in params:
                if param in c.Parameters:
                    if param == "F":
                        ...
                        # custom part to replace all z motion commands
                        # we only know z up and z down M3/m$ for now
                        # this may change later
                        # This looks to be what freecad creates
                        ...
                        if param == 'Z' and command in ['G0','G00','G1','G01']:
            ...
            if len(outstring) >= 1 and LAST_OUTSTRING != outstring:

                outline = format_outstring(outstring)

                if LAST_OUTSTRING != outline and outline[0] != '(':

                    #print( outline +" - "+ LAST_OUTSTRING )

                    LAST_OUTSTRING = outline
                    out += linenumber() + outline + "\n"
