...
struct StepControl {
    ...
    uint8_t isExecutingNonSysHold : 1;
    uint8_t isExecutingNonSysCorrection : 1;
};
...
typedef struct {
    ...
    bool           gedm_planner_line_running;
    bool           gedm_enable_gap_control;
    bool           gedm_single_axis_drill_task;
    bool           gedm_floating_z_gcode_task;
    bool           gedm_stepper_disable_z;
    bool           gedm_wire_gcode_task;
    bool           edm_process_finished;
    bool           edm_pause_motion;
    bool           edm_pause_motion_probe;
    bool           gedm_stop_process;
    bool           gedm_reset_protocol;
    bool           gedm_insert_probe;
    float          gedm_probe_position_x;
    float          gedm_probe_position_y;
    bool           gedm_reprobe_block;
    bool           gedm_reprobe_motion;
    bool           gedm_retraction_motion;
    bool           gedm_recover_motion;
    int            gedm_undo_moves;
    int            gedm_planner_sync;
} system_t;
extern system_t sys;
...
extern volatile ExecAccessory sys_rt_exec_accessory_override;  // Global realtime executor bitflag variable for spindle/co..olant overrides.
...
// Returns machine position of axis 'idx'. Must be sent the steps and the axis. No step array.
